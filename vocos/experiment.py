...
class VocosExp(pl.LightningModule):
    ...
    def __init__(
        ...
        if isinstance(self.head, ISTFTHead):
            self.stft = torchaudio.transforms.Spectrogram(
                n_fft=self.head.n_fft,
                hop_length=self.head.hop_length,
                win_length=self.head.win_length,
                power=None,
                return_complex=True,
            )
        else:
    ...
    def validation_step(self, batch, batch_idx, **kwargs):
        ...
        phase_diff_wrapped = None
        if self.stft is not None and isinstance(self.head, ISTFTHead):
            # Get predicted phase
            features = self.feature_extractor(audio_input, **kwargs)
            backbone_out = self.backbone(features, **kwargs)
            head_out = self.head.out(backbone_out).transpose(1, 2)
            _, pred_phase = head_out.chunk(2, dim=1)

            # Get true phase
            true_stft = self.stft(audio_input)
            true_phase = torch.angle(true_stft)

            # Calculate wrapped phase difference for visualization
            phase_diff = true_phase - pred_phase
            phase_diff_wrapped = torch.abs(phase_diff - 2 * math.pi * torch.round(phase_diff / (2 * math.pi)))
        ...
        return {
            ...
            "phase_diff_wrapped": phase_diff_wrapped[0] if phase_diff_wrapped is not None else None,
        }
    ...
    def validation_epoch_end(self, outputs):
        if self.global_rank == 0:
            ...
            phase_diff_wrapped = outputs[0]["phase_diff_wrapped"]
            self.logger.experiment.add_audio(
            ...
            if phase_diff_wrapped is not None:
                self.logger.experiment.add_image(
                    "val_wrapped_phase_difference",
                    plot_spectrogram_to_numpy(phase_diff_wrapped.data.cpu().numpy()),
                    self.global_step,
                    dataformats="HWC",
                )
