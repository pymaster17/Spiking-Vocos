...
class SpikingConvNeXtBlock(nn.Module):
    ...
    def forward(self, potential: torch.Tensor, cond_embedding_id: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        potential_flat = potential.flatten(0, 1)
        ...
        spikes = self.neuron2(h_spikes) # <--- 在GELU后生成新的脉冲
        spikes_flat = spikes.flatten(0,1)
        h_spikes = self.pwconv2(spikes_flat)

        # 幅度信息重注入
        # 通过克隆x_continuous来确保梯度能够正确传播
...
class SpikingVocosBackbone(Backbone):
    ...
    def __init__(
        ...
        self.membrane_output = MembraneOutputLayer(timestep=snn_timestep)
    ...
    def forward(
        ...
        # Reset SNN states before processing a new batch to handle variable sequence lengths
        ...
        for conv_block in self.convnext:
            ...
            if output_hidden_states:
                hidden_states.append(potential)
            
        # 4. 膜电位积分输出
        x = self.membrane_output(potential)
        
        # 5. 最终归一化和维度调整，以匹配head的输入要求
